...
Salida: resultados/controles.csv, con la misma estructura de columnas que
...
referencia del capitulo 6 (la media de |rho|), de modo que los numeros son
directamente comparables con los cuadros de ese capitulo.

Coste: la parte A-C no entrena ninguna red y tarda minutos. La ablacion D
entrena 96 redes (12 origenes x 4 horizontes x 2 semillas) y cuesta
aproximadamente la mitad de lo que costo la rejilla definitiva.
"""
...
def _fila(i_org, part, H, metodo, semilla, valores, mase_ref, n_ent, n_pru,
...
def main() -> int:
    ...
    for i_org, part in enumerate(particiones):
        ...
        for H in a.horizontes:
            muestras = V.construye_ventanas(norm, a.K, H, TAREA, paso=a.paso)
            ...
            pred_ref_ent = B.volatilidad_reciente(ent.X, H)
            escala = M.escala_referencia(ent.y, pred_ref_ent, ent.ticker)
            mase_ref = M.mase(pru.y, B.volatilidad_reciente(pru.X, H),
                              pru.ticker, escala)

            def anota(nombre, pred_pru, semilla=None, extra=None):
                filas.append(_fila(
                    i_org, part, H, nombre, semilla,
                    M.mase(pru.y, pred_pru, pru.ticker, escala),
                    mase_ref, len(ent), len(pru), extra))

            # --- referencia y controles de localizacion (A) ---------------
            anota(ref_nombre, B.volatilidad_reciente(pru.X, H))
            ...
            anota("media reescalada (c global)",
                  C.aplica_escala(B.volatilidad_reciente(pru.X, H), c_glob),
                  extra={"parametro": round(float(c_glob), 4)})

            c_serie = C.escalas_por_serie_l1(ent.y, pred_ref_ent, ent.ticker)
            anota("media reescalada (c por serie)",
                  C.aplica_escala(B.volatilidad_reciente(pru.X, H), c_serie,
                                  pru.ticker))

            # --- referencias del dominio (B) ------------------------------
            p_ewma_ent = C.ewma(ent.X, H, 0.94)
            c_ewma = C.escala_optima_l1(ent.y, p_ewma_ent)
            ...
            p_lam_ent = C.ewma(ent.X, H, lam)
            c_lam = C.escala_optima_l1(ent.y, p_lam_ent)
            anota("EWMA ajustado",
            ...
            if garch is not None:
                g_ent = garch.predice(ent.X, H)
                c_g = C.escala_optima_l1(ent.y, g_ent)
    ...
    print("\nresumen (media sobre origenes, por metodo y horizonte):")
    resumen = tabla.pivot_table(index="metodo", columns="H",
